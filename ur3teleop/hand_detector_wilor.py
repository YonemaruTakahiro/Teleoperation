...
class HandDetector_wilor:
    def __init__(self, device,model,model_cfg,renderer,detector,hand_type="Right",detect_hand_num=1,WIDTH=640,HEIGHT=480):
        self.device=device
        self.model=model
        ...
        if len(bboxes) != 0:
            ...
            for batch in dataloader:
                ...
                for n in range(batch_size):
                    ...
                    joints = out['pred_keypoints_3d'][n].detach().cpu().numpy()  ##関節の位置21個
                    ...
                    kpts_2d = self.keypoints_2d_on_image(joints, cam_t, scaled_focal_length)
            ...
            reconstructions = {'verts': all_verts, 'cam_t': all_cam_t, 'right': all_right, 'img_size': img_size[n],
                               'focal': scaled_focal_length, 'joints': all_joints, 'box_center': box_center,'rotmat':rotmat,'kpts_2d':kpts_2d}
    ...
    @staticmethod
    def rotmat_to_wrs(verts) -> np.ndarray:
    ...
    @staticmethod
    def fixed_rotmat_to_wrs(keypoint_3d_array):
        pos1 = np.array(keypoint_3d_array[4])
        pos2 = np.array(keypoint_3d_array[8])

        vert = pos2 - pos1

        x = np.array([vert[0], vert[1], 0])
        x = x / np.linalg.norm(x)
        y = np.array([x[1], -x[0], 0])

        tmp = np.array(keypoint_3d_array[17]) - np.array(keypoint_3d_array[5])
        if np.dot(y, tmp) < 0:
            y = -y
