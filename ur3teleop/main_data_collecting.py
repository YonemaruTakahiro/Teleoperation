...
def wilor_to_wrs(queue1: multiprocessing.Queue):
    ...
    hand_detector = HandDetector_wilor(device, model, model_cfg, renderer, detector, hand_type="Right",
                                       detect_hand_num=1, WIDTH=640, HEIGHT=480)
    ...
    # カメラのストリームを設定（RGBと深度）
    config_hand.enable_stream(rs.stream.color, WIDTH, HEIGHT, rs.format.bgr8, 60)  # 30は毎秒フレーム数
    config_hand.enable_stream(rs.stream.depth, WIDTH, HEIGHT, rs.format.z16, 60)
...
def wrs(queue1: multiprocessing.Queue,queue2: multiprocessing.Queue,queue3: multiprocessing.Queue):
    file_path="only_grouping_two_images.pkl"
        ...
        try:
            while True:
                ...
                if q is not None:
                    ...
                    # animation_data.current_jaw_width=robotx.get_jaw_width_realtime(rbt_socket)
                    # print(f"current_jaw_width:{animation_data.current_jaw_width}")
                    # ikからもとめた関節位置
                    jnt_values = robot.realtime_ik(animation_data.tgt_pos, animation_data.tgt_rotmat,
                ...
                if jnt_list is not None:
                    if jnt_list[0] is not None and jnt_list[1] is not None:
                        robotx.move_jspace_path_realtime(rbt_socket, path=jnt_list)
                        #一つのurscriptにエンドエフェクターとマニピュレータを動かすプログラムを書く必要がある。
                        # robotx.change_jaw_width_realtime(speed_percentage=100,force_percentage=50,finger_distance=(animation_data.jaw_width*85))
                ...
                if recording is True:
                    ...
                    if jnt_list is not None:
                        observation_robot = jnt_list[0]
                        this_episode_dict["observations"]["state"].append(observation_robot)
                        action = jnt_list[1]
