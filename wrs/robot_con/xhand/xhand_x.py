...
class XHandX:
    ...
    def send_command_and_get_hand_state(self, command, data=b""):
        """Send a generic command with a data payload."""
        if not self.ser or not self.ser.is_open:
            print("Error: Serial port is not open.")
            return None
        ...
        time.sleep(0.002)
        # Wait for response
        return self.read_actual_response()
    ...
    def read_actual_response(self):
        ...
        offset = 0
        for i in range(12):
            finger_state_data = data[offset:offset + 24]
            finger_states.append(xhand_bt.FingerState.from_bytes(finger_state_data))
            offset += 24
        # sensor data
        sensor_data_list = []
        for i in range(5):
            sensor_data_bytes = data[offset:offset + 384]
            sensor_data_list.append(xhand_bt.SensorData.from_bytes(sensor_data_bytes))
            offset += 384
    ...
    def goto_given_conf_and_get_hand_state(self, jnt_values):
        """Move all 12 fingers to given joint configurations."""
        if len(jnt_values) != 12:
            raise ValueError("Expected exactly 12 joint values.")
        # **Create FingerCommandPackage**
